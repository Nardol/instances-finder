...
#[derive(Parser)]
#[command(name = "xtask", version, about = "Dev helper for Instances Finder", propagate_version = true)]
struct Cli {
    #[command(subcommand)]
    cmd: Cmd,
}
...
enum ReleaseAction {
    ...
    Gh { version: Option<String>, #[arg(long)] notes: Option<String> },
}
...
fn ensure_cli() -> Result<()> {
    ...
    let (code, _) = run("node", &["node_modules/.bin/tauri", "--help"])?;
    if code != 0 {
        return Err(anyhow!("[ensure-cli] ✗ Tauri CLI not available after install"));
    }
...
fn doctor() -> Result<()> {
    ...
    match run("node", &["-v"]) {
        Ok((c, vout)) if c == 0 => {
            ...
            let major = v.split('.').next().unwrap_or("0").parse::<u64>().unwrap_or(0);
            if major >= 18 { mark("OK", &format!("Node.js {} (>=18)", v)); } else { mark("FAIL", &format!("Node.js {} (<18)", v)); }
        }
    ...
    if which::which("rustup").is_ok() {
        ...
        if c1 == 0 && clippy.contains("clippy") { mark("OK", "clippy installed"); } else { mark("WARN", "clippy missing (rustup component add clippy)"); }
        if c1 == 0 && clippy.contains("rustfmt") { mark("OK", "rustfmt installed"); } else { mark("WARN", "rustfmt missing (rustup component add rustfmt)"); }
    } else {
    ...
    if Path::new("node_modules/.bin/tauri").exists() {
        let (code, _) = run("node", &["node_modules/.bin/tauri", "--help"])?;
        if code == 0 { mark("OK", "Tauri CLI found (local devDependency)"); } else { mark("FAIL", "Tauri CLI present but not runnable"); }
    } else {
    ...
    if which::which("pkg-config").is_ok() {
        let ok41 = run("pkg-config", &["--exists", "webkit2gtk-4.1"]).map_or(false, |(c, _)| c == 0);
        let ok40 = run("pkg-config", &["--exists", "webkit2gtk-4.0"]).map_or(false, |(c, _)| c == 0);
        if ok41 || ok40 {
            let (code, ver) = if ok41 { run("pkg-config", &["--modversion", "webkit2gtk-4.1"]).unwrap_or((1, String::new())) } else { run("pkg-config", &["--modversion", "webkit2gtk-4.0"]).unwrap_or((1, String::new())) };
            if code == 0 { mark("OK", &format!("WebKitGTK dev found (webkit2gtk {})", ver.trim())); } else { mark("OK", "WebKitGTK dev found"); }
        } else {
    ...
    if which::which("dpkg").is_ok() {
        ...
        if missing.is_empty() {
            ...
            mark("WARN", &format!("Missing Debian build deps: {}", missing.join(", ")));
            println!("[doctor] Install example: sudo apt-get install -y {}", missing.join(" "));
        }
    }
    ...
    let is_win = os.contains("Windows_NT") || uname.contains("MINGW") || uname.contains("MSYS") || uname.contains("CYGWIN");
    if is_win {
        if which::which("where").is_ok() {
            let (c, _) = run("where", &["makensis"])?;
            if c == 0 { mark("OK", "NSIS (makensis) present"); } else { mark("WARN", "NSIS not found (optional unless building installer)"); }
        }
        if which::which("powershell.exe").is_ok() {
            ...
            if c == 0 && !name.trim().is_empty() { mark("OK", &format!("WebView2 Runtime detected ({})", name.trim())); } else { mark("WARN", "WebView2 Runtime not detected (bootstrapper Tauri will install if needed)"); }
        }
    }

    println!("\n[doctor] Summary: {} ok, {} warn, {} fail", ok, warn, fail);
    if fail > 0 { return Err(anyhow!("doctor: some checks failed")); }
    Ok(())
}
...
fn lint_cmd(fix: bool) -> Result<()> {
    if fix { npm_run("lint:fix", &[]) } else { npm_run("lint", &[]) }
}
...
fn check_cmd() -> Result<()> {
    ...
    let status = Command::new("cargo")
        .args(["clippy", "--manifest-path", "src-tauri/Cargo.toml", "--", "-D", "warnings"]) 
        .status()
        .context("cargo clippy failed")?;
    if !status.success() { return Err(anyhow!("clippy failed")); }
...
fn ci_checks_cmd() -> Result<()> {
    ...
    npm_run("lint", &[])?;
    // tsc --noEmit
    let (code, _out) = run("npx", &["--no-install", "tsc", "--noEmit"])?;
    if code != 0 { return Err(anyhow!("tsc --noEmit failed")); }
    // Prettier --check
    let (code2, _out2) = run("npx", &["--no-install", "prettier", "--check", "."])?;
    if code2 != 0 { return Err(anyhow!("prettier --check failed")); }
    // Clippy
    let status = Command::new("cargo")
        .args(["clippy", "--manifest-path", "src-tauri/Cargo.toml", "--", "-D", "warnings"]) 
        .status()
    ...
    if !status2.success() { return Err(anyhow!("rustfmt --check failed")); }
...
fn release_cmd(action: ReleaseAction) -> Result<()> {
    match action {
        ReleaseAction::Tag { version } => {
            let ver = version.or_else(|| env::var("VERSION").ok()).ok_or_else(|| anyhow!("Set VERSION or pass -- version"))?;
            npm_run("release:tag", &["--", &ver])
        }
        ReleaseAction::Gh { version, notes } => {
            let ver = version.or_else(|| env::var("VERSION").ok()).ok_or_else(|| anyhow!("Set VERSION or pass -- version"))?;
            let nts = notes.or_else(|| env::var("NOTES").ok()).unwrap_or_else(|| String::from("Release draft"));
...
fn fmt_check_cmd(which: FmtWhich) -> Result<()> {
    match which {
        FmtWhich::Js => {
            let (code, _out) = run("npx", &["--no-install", "prettier", "--check", "."])?;
            if code != 0 { return Err(anyhow!("prettier --check failed")); }
            Ok(())
        }
        FmtWhich::Rust => {
            let status = Command::new("cargo")
                ...
                .status()
                .context("cargo fmt --check failed")?;
            if !status.success() { return Err(anyhow!("rustfmt --check failed")); }
            Ok(())
        }
        FmtWhich::All => {
